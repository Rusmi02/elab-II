...
def build_graph(nodes, edges, service_points):
    ...
    for node_id, (x, y, square) in nodes.items():
        ...
        for sp_id, (sp_x, sp_y, _, _, _, _) in service_points.items():
            dist = calculate_distance((x, y), (sp_x, sp_y))
...
def plot_graph(G, nodes, edges, service_points):
...
def main():
    ...
    service_points = read_service_points("case II/Service_Point_Locations.csv")

    G = build_graph(nodes, edges, service_points)
    plot_graph(G, nodes, edges, service_points)
